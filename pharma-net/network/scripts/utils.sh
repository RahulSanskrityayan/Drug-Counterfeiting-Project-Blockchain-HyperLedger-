...
ORDERER_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/pharma-network.com/msp/tlscacerts/tlsca.pharma-network.com-cert.pem
  ...
  if [ "$ORG" == 'consumer' ]; then
    ...
    if [ "$PEER" -eq 0 ]; then
      ...
      if [ "$PEER" -eq 0 ]; then
        ...
        CORE_PEER_ADDRESS=peer1.distributor.pharma-network.com:13051
      fi
        ...
        if [ "$PEER" -eq 0 ]; then
          ...
          CORE_PEER_ADDRESS=peer1.manufacturer.pharma-network.com:14051
        fi
          ...
          if [ "$PEER" -eq 0 ]; then
            ...
            CORE_PEER_ADDRESS=peer1.retailer.pharma-network.com:15051
          fi
            ...
            if [ "$PEER" -eq 0 ]; then
              CORE_PEER_ADDRESS=peer0.transporter.pharma-network.com:12051
            else
...
instantiateChaincode() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer chaincode instantiate -o orderer.pharma-network.com:7050 -C "$CHANNEL_NAME" -n pharmanet -l "${LANGUAGE}" -v "${VERSION}" -c '{"Args":["org.pharma-network.com.pharmanet:instantiate"]}' -P "OR ('consumerMSP.member','distributorMSP.member','retailerMSP.member','transporterMSP.member','manufacturerMSP.member')" >&log.txt
    res=$?
    set +x
  else
    set -x
    peer chaincode instantiate -o orderer.pharma-network.com:7050 --tls "$CORE_PEER_TLS_ENABLED" --cafile $ORDERER_CA -C $CHANNEL_NAME -n pharmanet -l ${LANGUAGE} -v ${VERSION} -c '{"Args":["org.pharma-network.com.pharmanet:instantiate"]}' -P "OR ('consumerMSP.member','distributorMSP.member','retailerMSP.member','transporterMSP.member','manufacturerMSP.member')" >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
...
upgradeChaincode() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer chaincode upgrade -o orderer.pharma-network.com:7050 -C $CHANNEL_NAME -n pharmanet -l ${LANGUAGE} -v ${VERSION} -p ${CC_SRC_PATH} -c '{"Args":["org.pharma-network.com.pharmanet:instantiate"]}' -P "OR ('consumerMSP.member','distributorMSP.member','retailerMSP.member','transporterMSP.member','manufacturerMSP.member')" >&log.txt
    res=$?
    set +x
  else
    set -x
    peer chaincode upgrade -o orderer.pharma-network.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n pharmanet -l ${LANGUAGE} -v ${VERSION} -p ${CC_SRC_PATH} -c '{"Args":["org.pharma-network.com.pharmanet:instantiate"]}' -P "OR ('consumerMSP.member','distributorMSP.member','retailerMSP.member','transporterMSP.member','manufacturerMSP.member')" >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
